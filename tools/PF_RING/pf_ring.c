...
int lockless_list_add(lockless_list *l, void *elem) 
{
  ...
  /* TODO: 不是lockless的ring buffer实现吗？干嘛还加lock ? 
   * 说到底，这个ring buffer还不完全是lockless的 ! */
  ...
  for(i=0; i<MAX_NUM_LIST_ELEMENTS; i++) {
    ...
    /* 这里使用cmxchg指令来更新l->list_elements */
  ...
  /* 更新成功则增加一个elements
   * TODO: 由于l->list_elements[]数组的大小有限，理论上还是存在element没有添加成功的情况的
   * 为何这里还是简单的递增，而不判断i是否 >= MAX_NUM_LIST_ELEMENTS ? */
  l->num_elements++;
...
void lockless_list_empty(lockless_list *l, u_int8_t free_memory) 
{
  ...
  if(free_memory) {
    ...
    l->num_elements = 0;
    write_unlock_bh(&l->list_lock);
    wmb();  /* 使用write memory barrir 确保执行完成 */
  }
}
...
/* 收到一个数据包后，调用skb_ring_handler()进行处理 */
...
/* PF_RING类型socket的create函数，类似于inet_create() */
...
static int pf_ring_inject_packet_to_stack(struct net_device *netdev, struct msghdr *msg, size_t len) 
{
  ...
  err = netif_rx_ni(skb);  /* 通过netif_rx_ni()接口将skb注入kernel */ 
...
/* 发送skb，PF_RING socket也支持发送数据包！ 直接调用dev_queue_xmit() */
			     ...
			     zc_dev_mapping *mapping)
{
  ...
  if (mapping->operation == remove_device_mapping) {
	...
	if (unlikely(enable_debug))
	  printk("[PF_RING] %s:%d %s@%u removed mapping [num_bound_sockets=%u]\n", __FUNCTION__, __LINE__,
		 mapping->device_name, mapping->channel_id, entry->num_bound_sockets);
	...
	if (unlikely(enable_debug))
	  printk("[PF_RING] %s:%d %s@%d [num_bound_sockets=%d][%p]\n", __FUNCTION__, __LINE__,
	...
	if (!found) {
	  printk("[PF_RING] %s:%d something got wrong adding %s@%u %s\n", __FUNCTION__, __LINE__,
	...
	if (!found) {
	  printk("[PF_RING] %s:%d add %s@%u %s something got wrong (device not found)\n", __FUNCTION__, __LINE__,
	         mapping->device_name, mapping->channel_id, direction2string(pfr->mode));
	  return -1; /* Something got wrong */
	}

	if (unlikely(enable_debug))
	  printk("[PF_RING] %s:%d added mapping %s@%u [num_bound_sockets=%u]\n", __FUNCTION__, __LINE__,
		 mapping->device_name, mapping->channel_id, entry->num_bound_sockets);
...
static struct proto_ops ring_ops = {
  ...
  .ioctl = ring_ioctl,
  .recvmsg = ring_recvmsg,  /* 功能：查看收到了多少个包 */
  .sendmsg = ring_sendmsg,  /* 初步看下来，这个算是也能发送数据包 */
};
...
static int __init ring_init(void)
{
  ...
  ring_proc_init();
  sock_register(&ring_family_ops);  /* 新增PF_RING socket */
